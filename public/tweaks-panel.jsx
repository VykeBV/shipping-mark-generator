...
function TweakNumber({ label, value, min, max, step = 1, unit = '', onChange }) {
  // We keep an internal string state for the input so the user can naturally
  // clear and retype. The original implementation called `clamp(Number(...))`
  // on every keystroke, which meant typing "150" with a `min=20` snapped the
  // field back to "20" after every digit — you could never enter any partial
  // value smaller than min, even temporarily. Now: while focused, accept any
  // partial input; commit to parent only when the value parses to a finite
  // number, and clamp on blur (so the final committed value still respects
  // bounds).
  const clamp = (n) => {
  ...
  // Keep local in sync with parent when NOT actively editing (so drag-to-scrub
  // and external state updates both reflect in the field).
  React.useEffect(() => {
  ...
  const onScrubStart = (e) => {
    e.preventDefault();
    startRef.current = { x: e.clientX, val: Number(value) || 0 };
  ...
  const onInput = (e) => {
    ...
    setLocal(s);
    // Don't commit partial intermediates ("", "-", ".", "1." etc.) — the user
    // is still typing. Otherwise commit any finite number even if out of
    // range; we'll clamp on blur.
  ...
  const onBlur = () => {
    ...
    if (Number.isFinite(n)) {
      ...
      // Junk input — revert to parent value.
